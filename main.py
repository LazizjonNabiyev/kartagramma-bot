...
from keyboards import (
    city_selection_keyboard, transport_type_keyboard, back_button
)
from messages import WELCOME_MSG, CHOOSE_CITY_MSG, REMINDER_SET_MSG
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    reg = is_registered(message.from_user.id)
    extra = ""
    if not reg:
        extra = "\n\n📝 <b>Ro'yxatdan o'ting</b> — <i>/register</i>"

    await message.answer(
        WELCOME_MSG.format(name=message.from_user.first_name) + extra,
        reply_markup=main_menu_kb(),
        parse_mode="HTML"
    )


# ── Middleware: har xabarda last_active yangilash ─────────────────────────────
@dp.message.middleware()
async def track_activity(handler, message, data):
    uid = message.from_user.id
    update_last_active(uid)
    if message.text and not message.text.startswith("/"):
        log_action(uid, f"msg:{message.text[:30]}")
...
async def process_transport(message: Message, state: FSMContext):
    ...
    log_action(message.from_user.id, f"route:{data['from_stop']}->{data['to_stop']}")
...
async def reminder_route(message: Message, state: FSMContext):
    ...
    await message.answer("🔢 Necha bekatdan keyin tushishingiz kerak?\n<i>Masalan: 3</i>",
                         reply_markup=back_button(), parse_mode="HTML")
...
async def reminder_stops(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    data = await state.get_data()
    log_action(message.from_user.id, f"reminder:{data['route']}")
    await message.answer(
...
async def main():
    init_db()
    logger.info("🚀 Kartagramma Bot ishga tushdi!")
